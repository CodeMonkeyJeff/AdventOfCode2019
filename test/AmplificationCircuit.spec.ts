...
describe("Day 7 -- AmplificationCircuit", function() {
    ...
    describe("GetMaxPhaseSettingSequence", function() {
        ...
        tests.forEach((test): void => {
            it(test.AmpCode.join(", "), function(): void {
                const circuit = new AmplificationCircuit(test.AmpCode, { BreakOnOutput: test.isFeedbackEnabled });     // Turns on BRK on OUTPUT
    ...
    describe("CalculateThrustWithFeedback", function() {
        ...
        tests.forEach((test): void => {
            it(test.AmpCode.join(", "), function(): void {
                const circuit = new AmplificationCircuit(test.AmpCode, { BreakOnOutput: true });
