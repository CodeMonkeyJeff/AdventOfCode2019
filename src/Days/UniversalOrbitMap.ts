...
export class UniversalOrbitMap {
    ...
    private static readonly _input = [
        "2TJ)S3Z", "MJD)YP8", "9K5)7Q4", "HJK)ZKW", "6D4)P2Y", "VPM)CB4", "6FM)8V9", "QC8)KDV", "6JH)S5H", "SLF)MVN", "DM8)Y3H", "LKP)RVY",
    ...
    public getPath(spaceObject: string, endPoint = "COM"): string[] {
        if (spaceObject == endPoint) { return [endPoint]; }
        if (spaceObject == "COM") { return ["COM"]; }       // Safety for recursion

        const path: string[] = Array<string>();
        path.push(spaceObject);
        if (this._spaceObjects.includes(spaceObject)) { path.push(...this.getPath(this._orbits[spaceObject], endPoint)) }

        return path;
    }

    public countOrbits(spaceObject: string, endPoint = "COM"): number {
        if (spaceObject == endPoint) { return 0; }     // Base object for recursion
        if (this._spaceObjects.includes(spaceObject)) { return 1 + this.countOrbits(this._orbits[spaceObject], endPoint); }

        // Failsafe in case we get passed in a space object not orbiting a known value
        return 0;
    }

    private printPath(spaceObject: string): void { console.log(this.getPath(spaceObject).join(" ==> ")); }

    public static Day6Part1(): string {
        const map = new UniversalOrbitMap();
        map.parseOrbits(UniversalOrbitMap._input);
        return map.countAllOrbits().toString();
    }

    public static Day6Part2(): string {
        // The PLAN is to compute a path from ["SAN","YOU"] to "COM" and then to compute the space objects in common.
        // After that, we compute the distance from each object to each space object and pick the object with the least common distance.

        
        const map = new UniversalOrbitMap();
        map.parseOrbits(UniversalOrbitMap._input);

        const santaPath = map.getPath("SAN");
        const youPath = map.getPath("YOU");
        const commonSpaceObjects = santaPath.filter((spaceObject: string): boolean => { return youPath.includes(spaceObject); });        // Calculate space objects in common
        const currentClosesSpaceObject = { spaceObject: "COM", distance: 295834 };       // Absurd value

        commonSpaceObjects.forEach((spaceObject: string): void => {
            const fromSAN = map.countOrbits("SAN", spaceObject);
            const fromYOU = map.countOrbits("YOU", spaceObject);

            if (fromSAN + fromYOU < currentClosesSpaceObject.distance) {                
                currentClosesSpaceObject.spaceObject = spaceObject;
                currentClosesSpaceObject.distance = fromSAN + fromYOU - 2;
            }
        });

        return (currentClosesSpaceObject.distance).toString();
    }
}