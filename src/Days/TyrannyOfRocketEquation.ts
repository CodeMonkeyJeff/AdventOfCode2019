...
export class TyrannyOfRocketEquation {
    ...
    public static Day1Part1(): string { return TyrannyOfRocketEquation._input.map((val: number): number => TyrannyOfRocketEquation.GetFuel(val)).reduce((total: number, val: number) => total + val).toString(); }
    public static Day1Part2(): string { return TyrannyOfRocketEquation._input.map((val: number) => TyrannyOfRocketEquation.GetRecursiveFuel(val)).reduce((total: number, val: number) => total + val).toString(); }
}