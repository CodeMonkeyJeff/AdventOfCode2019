...
export class ProgramAlarm1202 {
    ...
    public static Day2Part1(): string {
        const tape = this.GetTestMachine(12, 2);
    ...
    public static Day2Part2(): string {
        ...
        while (inputs.noun <= 99) {
            while (inputs.verb <= 99) {
                const tape = this.GetTestMachine(inputs.noun, inputs.verb);
                const output: number = tape.ExecuteTape()[0];
                if (output == goal) { return this.CalculateSubmission(inputs.noun, inputs.verb).toString(); }
                inputs.verb++;
            }
            inputs.verb = 0;
            inputs.noun++;
    ...
    private static GetTestMachine(noun: number, verb: number): IntcodeMachine {
        const tape = new IntcodeMachine(this._input);
        tape.Tape[1] = noun;
        tape.Tape[2] = verb;
        return tape;
    }
