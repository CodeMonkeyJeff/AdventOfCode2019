...
export class SetAndForget {
    ...
    public ParseGrid(): SetAndForget {
        // ^ 94     v 76    < 60    > 62    # 35
        const isScaffold = (v: bigint): boolean => v == 35n || v == 94n || v == 76n || v == 60n || v == 62n;
        const isAdjacent = (a: Scaffold, b: Scaffold): boolean => Math.abs(a.x - b.x) + Math.abs(a.y - b.y) == 1;
        const getNeighbors = (p: Scaffold): Scaffold[] => this._scaffolds.filter(v => isAdjacent(v,p));
        
        // The grid is square, delimited by ASCII newlines
        const lineLength = this._output.findIndex(v => v == 10n) + 1;

        // Parsing out scaffolding
        this._output.forEach((v: bigint, i: number): void => {
            if (isScaffold(v)) {
                const dy = Math.floor(i / lineLength);
                const dx = i % lineLength;       // Getting rid of all the newlines too
                const scaffold = SetAndForget.CreateScaffold(dx, dy, i);                
                this._scaffolds.push(scaffold);
            }
        });

        // Computing neighbors (needs to be done after parsing out the scaffolds)
        this._scaffolds.forEach(v => { v.neighbors = getNeighbors(v); });

        // Marking the intersections with 'O' == 79n
        const intersections = this._scaffolds.filter(v => v.neighbors.length > 2);
        intersections.forEach(v => { this._output[v.index] = 79n; });
    ...
    public static CreateScaffold(x: number, y: number, i: number): Scaffold { return { x: x, y: y, index: i, neighbors: new Array<Scaffold>() }; }

    public static Day17Part1(): string {
        const saf = new SetAndForget().Execute().ParseGrid();
        const intersections = saf._scaffolds.filter(v => v.neighbors.length > 2);
